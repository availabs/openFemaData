...
--         WHERE (
-- --             swd_property_damage > 0 OR swd_crop_damage > 0 OR injuries_direct > 0 OR injuries_indirect > 0 OR deaths_direct > 0 OR deaths_indirect > 0 OR
-- --             fema_property_damage > 0 OR fema_crop_damage > 0
--             )
        GROUP BY 1, 2, 3
    ),
    details_fema_per_basis as (
        SELECT generate_series(coalesce(swd_begin_date, fema_incident_begin_date)::date,
                               LEAST(
                                   ...
                                   ), '1 day'::interval)::date begin_date,
               nri_category hazard,
               geoid,
...
-- 			episode_id,
            nri_category hazard,
            geoid,
            coalesce(swd_begin_date::date, fema_incident_begin_date::date) begin_date,
        ...
        from fusion_events
        where nri_category NOT IN ('coldwave', 'drought', 'heatwave', 'icestorm', 'riverine', 'winterweat', 'Northern Lights')
        group by  1, 2, 3
    ),
per_basis as (
	select 'agg' src, hazard nri_category, geoid, event_ids,num_events, begin_date,
		   swd_property_damage, swd_crop_damage, fema_property_damage, fema_crop_damage, fatalities_dollar_value
	-- 	   coalesce(swd_property_damage, fema_property_damage) merged_property_damage,
	-- 	   coalesce(swd_crop_damage, fema_crop_damage)         merged_crop_damage
	from aggregation

	UNION ALL

	select 'pb' src, hazard nri_category, geoid, event_ids,num_events, begin_date,
	       swd_property_damage, swd_crop_damage, fema_property_damage, fema_crop_damage, fatalities_dollar_value
	-- 	   coalesce(swd_property_damage, fema_property_damage) merged_property_damage,
	-- 	   coalesce(swd_crop_damage, fema_crop_damage)         merged_crop_damage
...
select * 
--into public.temp_per_basis_v1_0
from per_basis
-- select nri_category, count(1), sum(
-- 	case 
-- 		when coalesce(swd_property_damage,0) + coalesce(swd_crop_damage,0) + coalesce(fatalities_dollar_value,0) > 0 Then 1
-- 		else 0 
--     end
-- ) count_swd_events,
-- sum(
