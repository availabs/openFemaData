...
-- where event_id IN (5536875, 5537083)
with dollars as (SELECT nri_category,
                        ...
                        CASE
                            ...
                            WHEN ctype = 'population'
                                ...
                                END)
                            END avail
                 FROM tmp_hlr_normalised_pop_v2
     ...
     nri as (
         ...
         from  national_risk_index.nri_counties_november_2021,
               (select unnest(array['avalanche',
                   ...
                   'wildfire',
                   'wind',
                   'winterweat']) nri_category) nri_category
         group by 1
         order by 1
     )

select nri_category, to_char(swd_buildings, '$ 999,999,999,999') swd_buildings,
       to_char(swd_crop, '$ 999,999,999,999') swd_crop,
       to_char(swd_population, '$ 999,999,999,999') swd_population,
       to_char(buildings, '$ 999,999,999,999') buildings,
       to_char(crop, '$ 999,999,999,999') crop,
       to_char(pop, '$ 999,999,999,999') pop,
       round((((swd_buildings - buildings) / nullif(buildings, 0)) * 100)::numeric, 2) b_diff,
       round((((swd_crop - crop) / nullif(crop, 0)) * 100)::numeric, 2) c_diff,
       round((((swd_population - pop) / nullif(pop, 0)) * 100)::numeric, 2) p_diff
from avail join nri
                using (nri_category)
