...
--              Where substring(geoid, 1, 5) = '12087'
     ...
     tmp_calcs as (SELECT
                       disaster_number, geoid, nri_category,
...
select nri_category,
       sum(fema_property_damage) fema_property_damage, sum(swd_property_damage) swd_property_damage,
       sum(fema_crop_damage) fema_crop_damage, sum(swd_crop_damage) swd_crop_damage
from tmp_calcs
group by 1;
