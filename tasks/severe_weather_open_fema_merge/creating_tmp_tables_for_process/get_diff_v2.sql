with per_basis as (
    select * from public.tmp_per_basis_data_zero_loss_detailed_new_data
),
     lrpbs as (
     ...
     t as (select geoid,
                  ...
                  (
                          COALESCE((1.0 / NULLIF(b_va_r, 0)) / nullif(wt_denom,0) *
                                   COALESCE(((1.0 / NULLIF(b_va_r, 0)) / nullif(wt_denom,0) ) * b_av_r, 0),0) +
                          COALESCE((1.0 / NULLIF(b_va_s, 0)) / nullif(wt_denom,0) *
                                   COALESCE(((1.0 / NULLIF(b_va_s, 0)) / nullif(wt_denom,0) ) * b_av_s, 0),0) +
                          COALESCE((1.0 / NULLIF(b_va_c, 0)) / nullif(wt_denom,0) *
                                   COALESCE(((1.0 / NULLIF(b_va_c, 0)) / nullif(wt_denom,0) ) * b_av_c, 0),0)
                  ...
                  Coalesce(Round((((
                                       ...
                                       ) - b.nri_hlr_b) / NULLIF(b.nri_hlr_b,0) ) * 100),0) as hlr_diff,
                  ...
                  round((
                            ...
                            ) * b.nri_exp_b * nri_freq) as swd_cost,
                  Coalesce(round((
                                         COALESCE((1.0 / NULLIF(b_va_r, 0)) / nullif(wt_denom,0) *
                                                  COALESCE(((1.0 / NULLIF(b_va_r, 0)) / nullif(wt_denom,0) ) * b_av_r, 0),0) +
                                         COALESCE((1.0 / NULLIF(b_va_s, 0)) / nullif(wt_denom,0) *
                                     ...
                                     ) * b.nri_exp_b * nri_freq) - round(b.nri_hlr_b * b.nri_exp_b * nri_freq)
                               / nullif(round(b.nri_hlr_b * b.nri_exp_b * nri_freq),0),0) * 100 as cost_diff
     ...
     diff AS (
         SELECT hlr.*, cfld_hlrb,
                ((hlr_b - cfld_hlrb) / nullif(cfld_hlrb, 0)) * 100 percent_hlr_diff,
                hlr_b * cfld_expb * cfld_afreq our_total_cfld,
                hlr_b * rfld_expb * rfld_afreq our_total_rfld,
                hlr_b * hrcn_expb * hrcn_afreq our_total_hrcn,

                cfld_hlrb * cfld_expb * cfld_afreq nri_total_cfld,
                rfld_hlrb * rfld_expb * rfld_afreq nri_total_rfld,
                hrcn_hlrb * hrcn_expb * hrcn_afreq nri_total_hrcn
         FROM tmp_hlr_new_merged_data hlr
...
SELECT diff.geoid,
       state,
       cz_name,
       to_char(our_total_cfld, 'FM9,999,999,999.99') our_total_cfld,
       to_char(nri_total_cfld, 'FM9,999,999,999.99') nri_total_cfld,

       to_char(our_total_rfld, 'FM9,999,999,999.99') our_total_rfld,
       to_char(nri_total_rfld, 'FM9,999,999,999.99') nri_total_rfld,

       to_char(our_total_hrcn, 'FM9,999,999,999.99') our_total_hrcn,
       to_char(nri_total_hrcn, 'FM9,999,999,999.99') nri_total_hrcn,

       to_char(property_damage, 'FM9,999,999,999') property_damage

FROM severe_weather_new.details
         JOIN diff
              ON diff.geoid = substring(details.geoid, 1, 5)
                  AND nri_category = event_type_formatted
                  AND percent_hlr_diff is not null
WHERE event_type_formatted IN ('coastal', 'hurricane', 'riverine')
  AND year >= 1996 and year <= 2019
--   and diff.geoid like '22%'
  and diff.geoid in (select geoid from t limit 50)
ORDER BY event_type_formatted, percent_hlr_diff DESC
